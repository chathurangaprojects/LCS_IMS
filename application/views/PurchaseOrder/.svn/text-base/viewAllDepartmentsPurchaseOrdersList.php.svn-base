    ...
    <table id="sort-table">
	  ...
	  ?>
        ...
        <tr>
          ...
          <td><?php if(trim($PurchaseOrders[$index]->getPO_Purpose())!=""){echo $PurchaseOrders[$index]->getPO_Purpose(); } else{ echo "-"; }?></td>
          <td><?php echo $poGeneral->retrievePoStatus($PurchaseOrders[$index]->getStatus_Code()); ?></td>
           
          <td>
          <a title="Edit this example" href="<?php  echo base_url(); ?>index.php/PurchaseOrder/PurchaseOrderManagement/viewPurchaseOrderRequest/<?php echo $PurchaseOrders[$index]->getOrder_Code(); ?>" onclick="return confirm('Are you sure to edit this Purchase Order request? ');retrieveItemDetails();" > <!--<span class="ui-icon ui-icon-wrench"></span> -->
